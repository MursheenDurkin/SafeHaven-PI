...
show_help() {
    ...
    echo ""
    echo -e "  ${GREY}Once inside the menu — modes:${RESET}"
    echo -e "  ${WHITE}[1]${RESET}    Traveler  — everyday public-WiFi protection"
    echo -e "  ${WHITE}[2]${RESET}    Activist  — Tor routing, zero logs"
    echo -e "  ${WHITE}[3]${RESET}    Business  — captive portal + per-user admin"
    echo -e "  ${WHITE}[4]${RESET}    Relaxed   — full stack minus VPN"
    echo ""
    echo -e "  ${GREY}Tools:${RESET}"
    echo -e "  ${WHITE}[5]${RESET}    Live security logs viewer"
    echo -e "  ${WHITE}[6]${RESET}    Manage VPN devices (add / list / edit / remove)"
    echo -e "  ${WHITE}[7]${RESET}    Pi-hole DNS block statistics"
    echo -e "  ${WHITE}[8]${RESET}    Open the web dashboard"
    echo -e "  ${WHITE}[9]${RESET}    Mobile access (Termux) guide"
    echo -e "  ${WHITE}[0]${RESET}    Export security report (last 24h to file)"
    echo ""
    echo -e "  ${GREY}System:${RESET}"
    echo -e "  ${WHITE}[w]${RESET}    Setup Wizard (hotspot, hostname, password, VPN keys)"
